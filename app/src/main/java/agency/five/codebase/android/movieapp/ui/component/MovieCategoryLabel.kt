...
@Composable
fun MovieCategoryLabel(movieCategoryLabelViewState: MovieCategoryLabelViewState){
    if(movieCategoryLabelViewState.isSelected){
        SelectedMovieCategory(movieCategoryLabelViewState)
    }
    else{
...
@Composable
fun SelectedMovieCategory(movieCategoryLabelViewState: MovieCategoryLabelViewState){
    Column(modifier = Modifier.width(intrinsicSize = IntrinsicSize.Max)) {
        Text(text = selectTextSource(movieCategoryLabelViewState = movieCategoryLabelViewState))
        Spacer(modifier = Modifier.size(5.dp))
        Divider(color = Color.Black, modifier = Modifier.fillMaxWidth(), thickness = 3.dp)
    }
}

@Composable
fun UnselectedMovieCategory(movieCategoryLabelViewState: MovieCategoryLabelViewState,modifier: Modifier= Modifier){
    Text(text = selectTextSource(movieCategoryLabelViewState = movieCategoryLabelViewState), modifier.alpha(ContentAlpha.disabled))
}
